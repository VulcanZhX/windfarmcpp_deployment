...
WindFarmOptimization::WindFarmOptimization(
	...
	const std::vector<std::vector<double>>& efficiency_all_value,
    const std::vector<int>& turbine_status, // from 0 to 34, reduced to binary array(14->1, others->0)
    ...
    std::vector<std::vector<double>> efficiency_all = efficiency_all_value; // free memory
	int count_tn = 1; // label
    // reduce turbine_status to binary status

	for (auto& elem : turbine_chart_input) {
		elem = Turbine(PT, t_qz, turbulence_sheer_veer, turbine_diameter_vector, 
            turbine_hub_height_vector, rated_power_vector, life_total_vector, 
            repair_c_vector, fatigue, fatigue_p, efficiency_all[count_tn - 1], turbine_status[count_tn - 1], count_tn);
		elem.yaw_angle = yaw_vec[count_tn - 1];
		count_tn++;
	}
	turbine_chart = turbine_chart_input;
    // startup_turbine_idx = ; // status = 1 turbine idx storage
...
void WindFarmOptimization::calculateWake() {
	...
	for (int i = 0; i < n_turbines; i++) {
        ...
        // check whether turbine is active (1)
...
// optim_objective
// add start_stop status check later
// 获取所有风机偏航角
