...
public:
    ...
    void calculateWake();
    // 计算尾流影响矩阵
    Eigen::MatrixXi calculate_wake_matrix();

    // 其他接口和成员函数根据需要补充...
    // 风机偏航梯度计算, mode=0: P_all, mode=1: P_qz12, mode=2: P_qz3
    // double grad_p_gamma_single(int kIdx, const Eigen::MatrixXi &wake_matrix, int mode);
...
// 判断风机尾流重叠
int calculate_overlap(const Eigen::VectorXd &undisturbed_velocity, const Eigen::VectorXd &disturbed_velocity, const Turbine &turbine);
